...
block::block( cocos2d::Layer* layer )
{
    ...
    block1->setScale(visibleSize.width/sizeblock.x*0.10,visibleSize.height/sizeblock.y*0.15);

    //block1->setPosition( Vec2( visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y ) );
    block1->setPosition(Vec2(visibleSize.width/(float)((float)1/(float)cocos2d::RandomHelper::random_int(1, 8))+visibleSize.width,(visibleSize.height/2)*0.35));
